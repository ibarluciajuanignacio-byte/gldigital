...
export function DarkModeToggle({ checked, onChange, "aria-label": ariaLabel = "Cambiar a modo oscuro o claro" }: Props) {
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    onChange(e.target.checked);
  };

  return (
    <div className="silva-darkmode-toggle-wrap">
      <input
        type="checkbox"
        id="darkmode-toggle"
        className="silva-darkmode-toggle-input"
        checked={checked}
        onChange={handleChange}
        aria-label={ariaLabel}
      />
      <label htmlFor="darkmode-toggle" className="silva-darkmode-toggle-label">
        <svg className="sun" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 496 496" aria-hidden>
        ...
        <svg className="moon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 49.739 49.739" aria-hidden>
